...
{   
    public class DestroyManager : MonoBehaviourSingleton<DestroyManager>
    {
        ...
        public List<GameObject> destoryGameObjects = new List<GameObject>();//파괴할 오브젝트 리스트
        ...
        private Transform tmpParent;//캐싱용
        ...
        {
            base.Awake();
            Timing.RunCoroutine(UpdateDestroyManager());
            tmpParent = transform;
            lLcroweUtil.DontDestroyTargetObject(gameObject);
        }

        /// <summary> 업데이트 모듈에 집어넣는것</summary>
        private IEnumerator<float> UpdateDestroyManager()
        {
            //주석처리하고 고쳐야됨
            if (destoryGameObjects.Count < destroyCount)
            {
                for (int i = 0; i < destoryGameObjects.Count; i++)
                {
                    _targetDestroy = destoryGameObjects[i];
                    if (!ReferenceEquals(_targetDestroy, null))
                    {
                        destoryGameObjects.Remove(_targetDestroy);

                        //뭔가 애매한 지역이다 현재테스트할때는 문제없다.
                        //어디서 인터페이스를 사용한후 버그가 난다.
                        //어설프게 최적화를 하는것보다는 GC를 믿어보는게 좋을거 같다.
        ...
        public void AddDestoryGameObject(GameObject _targetObject)
        {
            ...
            if (!destoryGameObjects.Contains(_targetObject))
            {
                _targetObject.transform.SetParent(tmpParent);
                destoryGameObjects.Add(_targetObject);
        ...
        public void AddWaitDestoryGameObject(GameObject _targetObject)
        {
            ...
            {
                _targetObject.SetActive(false);
                _targetObject.transform.parent = tmpParent;
                Timing.RunCoroutine(WaitDeactiveGameObject(_targetObject));
            }
        }
        private IEnumerator<float> WaitDeactiveGameObject(GameObject _gameObject)
        {
            ...
            yield return Timing.WaitForSeconds((tempIndex * (Time.timeScale * 100f)) * 0.01f);//신규
            //(5*(1*100))*0.01f = 5sec
            destoryGameObjects.Add(_gameObject);
        }
