...
namespace lLCroweTool.ToolTipSystem
{
    ...
    public class ToolTipUiView : MonoBehaviour
    {
        ...
        {   
            ...
            Vector2 anchorPos = MousePointer.Instance.mouseScreenPosition / toolTipView.localScale.x + offsetPos;
        ...
        public void MoveToolTip(Transform _pos)
        {
            if (_pos != null)
            {
                toolTipView.position = (Vector2)_pos.position + offsetPos;
            }
            CheckSidePos(toolTipView.position);
        ...
        private void CheckSidePos(Vector2 _anchorPos)
        {
            //외곽체크
            //스크린 X
            float newX = _anchorPos.x + toolTipView.rect.width > Screen.width ? Screen.width - toolTipView.rect.width : _anchorPos.x;
            _anchorPos.x = Mathf.Clamp(_anchorPos.x, 0, newX);//스크린 왼쪽 사이드

            //if (_anchorPos.x + toolTipView.rect.width > Screen.width)
            //{
            //    _anchorPos.x = Screen.width - toolTipView.rect.width;
            //}
            ...
            //스크린 Y
            float newY = _anchorPos.y + toolTipView.rect.height > Screen.height ? Screen.height - toolTipView.rect.height : _anchorPos.y;
            _anchorPos.y = Mathf.Clamp(_anchorPos.y, 0, newY);

            ////스크린 위 사이드
            //if (_anchorPos.y + toolTipView.rect.height > Screen.height)
            //{
            //    _anchorPos.y = Screen.height - toolTipView.rect.height;
            //}
            ////스크린 아래 사이드
            ...
            //    _anchorPos.y = 0;
            //}
            toolTipView.position = _anchorPos;
        }
    }
}